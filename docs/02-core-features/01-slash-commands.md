...
- **백그라운드 세션**(`claude --bg`)도 같은 목록에 `bg`로 표시되어 재개할 수 있습니다 (2.1.144). 여러 세션을 한눈에 관리하려면 `claude agents` (Agent View)를 사용하세요
...
- **`/clear` 이전 지점부터 대화 재개** (2.1.191): `/clear`로 대화를 비운 뒤에도 `/rewind`로 초기화 이전 시점을 선택해 대화를 이어갈 수 있습니다
...
**프롬프트에서 직접 설정 (2.1.181)**: `/config key=value` 문법으로 설정 화면을 열지 않고 값을 지정할 수 있습니다. 지정 가능한 단축 키 목록은 `/config --help`로 확인합니다 (2.1.183).

```
> /config theme=dark
> /config --help          # 설정 가능한 key 목록
```
...
- Opus 선택 시 좌/우 화살표로 **노력 수준** (effort level)을 조절할 수 있습니다
- 변경은 즉시 적용됩니다
- **선택한 모델이 새 세션의 기본값으로 저장**됩니다 (2.1.153, IDE와 동일). 현재 세션에만 적용하려면 선택기에서 **`s` 키**를 누르세요 (이전 키바인딩 `modelPicker:setAsDefault` → `modelPicker:thisSessionOnly`로 변경)
...
- **xhigh**는 Opus(4.7/4.8)에서 가장 어려운 작업에 권장되는 수준입니다 (기본은 high)
- **`ultracode`** (2.1.160): xHigh를 지원하는 모델에서 제공되는 **최상위 노력 레벨**입니다. `/effort ultracode`를 지정하면 다이나믹 워크플로우가 발동하며, xHigh 미지원 모델에서는 이 레벨이 제공되지 않습니다. 프롬프트의 트리거 키워드도 `workflow`에서 `ultracode`로 변경되었습니다
- `auto`로 두면 작업에 따라 효율적인 수준이 자동 선택됩니다
- 모델 선택기의 좌/우 화살표와 동일한 효과입니다 (2.1.154에서 슬라이더 레이블 "Speed"/"Intelligence" → **"Faster"/"Smarter"**)
...
### `/scroll-speed` — 마우스 휠 스크롤 속도 조절

마우스 휠 스크롤 속도를 라이브 프리뷰와 함께 조정합니다 (2.1.139).

```
> /scroll-speed
```

- 좌/우 화살표로 속도를 바꾸면 즉시 미리보기로 확인할 수 있습니다
- 트랜스크립트가 너무 빠르게/느리게 스크롤될 때 유용합니다
...
- **빠른 단일 패스 리뷰 (2.1.202)**: `/review <pr>`은 2.1.186에서 잠시 `/code-review medium` 엔진을 공유했다가, 2.1.202에서 다시 **빠른 단일 패스** 방식으로 되돌아왔습니다. 멀티에이전트 심층 리뷰가 필요하면 `/code-review <노력> <PR번호>`를 사용하세요
...
### `/cd` — 작업 디렉토리 이동 (2.1.169)

세션의 작업 디렉토리를 다른 경로로 이동합니다.

```
> /cd ../other-service
```

- **프롬프트 캐시를 유지한 채** 디렉토리를 옮기므로, 세션을 새로 시작하지 않고도 다른 하위 프로젝트로 전환할 수 있습니다
- 모노레포에서 여러 패키지를 오가며 작업할 때 유용합니다
...
- **카테고리별 분석 (2.1.149)**: 한도 소비를 **스킬·서브에이전트·플러그인·MCP 서버별 비용**으로 세분화해 표시합니다. 2.1.152부터는 대용량 세션 파일도 분석 항목에 포함됩니다(스트리밍 방식으로 메모리 사용은 일정 유지).
...
완료 조건(목표)을 설정하면 Claude가 그 조건이 충족될 때까지 여러 턴에 걸쳐 작업을 이어갑니다 (2.1.139).

```
> /goal 모든 테스트가 통과할 때까지 수정해줘
> /goal CI가 초록불이 되면 멈춰
```

- `/loop`가 정해진 **간격**으로 반복한다면, `/goal`은 **조건**이 충족될 때까지 자율적으로 진행합니다
- 목표 달성 여부는 매 턴 후 평가됩니다 (백그라운드 셸/서브에이전트 실행 중에는 평가를 보류)
- 무한 루프를 막기 위해 사용량과 진행 상황을 함께 고려합니다

### `/workflows` — 다이나믹 워크플로우 (2.1.154)

복잡하거나 대규모인 작업을 Claude가 **수십~수백 개의 에이전트로 분해해 백그라운드에서 오케스트레이션**하도록 요청하는 기능입니다.

```
> 이 워크플로우를 만들어줘: ...   # 자연어로 워크플로우 생성 요청
> /workflows                       # 진행 중/완료된 워크플로우 실행 목록 확인
```

- 트리거 키워드 **`ultracode`**(2.1.160, 이전 이름 `workflow`)로 요청하면 Claude가 에이전트 함대를 구성해 작업을 병렬 처리합니다. `ultracode`는 `/effort`의 최상위 노력 레벨이기도 하며 xHigh를 지원하는 모델에서만 제공됩니다
- 단일 컨텍스트로는 벅찬 대규모 마이그레이션·감사·일괄 작업에 적합합니다
- 진행 상황은 라이브 에이전트 수 카운트로 상태 표시줄에 표시되며, 응답 후 "Waiting for N background agents/workflows" 타이머로 추적됩니다 (2.1.152)
- 워크플로우 크기 권고치(small/medium/large)는 `/config`의 **"Dynamic workflow size"** 설정으로 조절합니다 (2.1.202)
- `/workflows` 상세 뷰에서 **`f` 키로 에이전트를 상태별로 필터링**할 수 있습니다 (2.1.186)

### `/reload-skills` — 스킬 재스캔 (2.1.152)

재시작 없이 스킬 디렉토리를 다시 스캔하여 새로 추가/수정한 스킬을 현재 세션에서 사용할 수 있게 합니다.

```
> /reload-skills
```

- 스킬을 만들거나 편집한 직후 바로 적용할 때 유용합니다
- `SessionStart` 훅에서 `reloadSkills: true`를 반환해도 동일하게 재스캔됩니다 ([20장](../05-advanced/02-hooks.md) 참고)
...
### `/code-review` — 코드 리뷰 + 품질 개선 (이전 이름 `/simplify`)

최근 변경한 파일을 병렬 에이전트로 리뷰합니다. **2.1.147부터 `/simplify`가 `/code-review`로 이름이 바뀌었고**, 단순 정리를 넘어 **정확성 버그(correctness bugs)**까지 지정한 노력 수준으로 보고합니다.

```
> /code-review
> /code-review high              # 노력 수준 지정 (더 깊은 리뷰)
> /code-review --comment         # GitHub PR에 인라인 코멘트로 게시
> /code-review --fix             # 리뷰 결과를 워킹 트리에 직접 적용 (2.1.152)
...
| 에이전트 | 검사 항목 |
|----------|----------|
| **정확성** | 로직 버그, 엣지 케이스, 회귀 (2.1.147 추가) |
| **코드 재사용** | 중복 코드, 기존 유틸리티 활용 |
| **코드 품질** | 가독성, 일관성, 베스트 프랙티스 |
| **효율성** | 성능, 메모리, 불필요한 연산 |

- `high` 등 **노력 수준**을 인자로 주면 더 철저하게 검토합니다 (`low`/`medium`은 고신뢰 소수 발견, `high`~`max`는 넓은 커버리지, `ultra`는 클라우드 다중 에이전트 심층 리뷰)
- `--comment`를 붙이면 결과를 **GitHub PR 인라인 코멘트**로 직접 남깁니다
- `--fix`를 붙이면 리뷰 후 발견 사항을 **워킹 트리에 바로 적용**합니다 (2.1.152)
- 선택적으로 포커스 지침을 전달하여 특정 관점에 집중할 수 있습니다
- 기능 구현이나 버그 수정 후 **마무리 단계**에서 사용하기 좋습니다

### `/simplify` — 품질 정리 전용 리뷰 (재설계, 2.1.154)

코드 재사용·단순화·효율성·추상화 수준(altitude) 관점의 **정리(cleanup) 리뷰만** 수행하고 수정을 적용합니다.

```
> /simplify
```

- 내부적으로 `/code-review --fix`를 호출하지만 **버그 탐색은 하지 않고** 품질 정리에만 집중합니다
- 버그까지 잡으려면 `/code-review`를 사용하세요
- 2.1.147에서 `/code-review`로 이름이 바뀌었다가, 2.1.154에서 품질 전용 명령어로 **다시 분리**되었습니다
...
### `/dataviz` — 데이터 시각화 스킬 (2.1.198)

차트와 대시보드를 디자인하는 내장 스킬입니다.

```
> /dataviz 월별 매출 데이터를 대시보드로 만들어줘
```

- 색상 팔레트 **검증기**가 포함되어 접근성·대비를 확인하며 시각화를 구성합니다
- 데이터 리포트, 지표 대시보드 등 시각적 산출물 제작에 적합합니다
...
| | `/review` | `[pr]` | PR 리뷰 (빠른 단일 패스, 2.1.202) |
| | `/pr-comments` | — | PR 코멘트 가져오기 |
| | `/add-dir` | `<경로>` | 작업 디렉토리 추가 |
| | `/cd` | `<경로>` | 작업 디렉토리 이동 (프롬프트 캐시 유지, 2.1.169) |
| | `/plan` | `[설명]` | Plan 모드 진입 |
| **정보** | `/help` | — | 도움말 |
...
| | `/goal` | `<완료 조건>` | 조건 충족까지 자율 진행 |
| | `/workflows` | — | 다이나믹 워크플로우 실행 목록 |
| | `/reload-skills` | — | 스킬 디렉토리 재스캔 |
| **음성** | `/voice` | — | Voice Mode 토글 |
| **번들** | `/code-review` | `[노력\|--comment\|--fix]` | 코드 리뷰(정확성 버그 포함) |
| | `/simplify` | — | 품질 정리 전용 리뷰 + 수정 적용 |
| | `/dataviz` | `[설명]` | 데이터 시각화 스킬 (색상 팔레트 검증기, 2.1.198) |
| | `/batch` | `<설명>` | 대규모 병렬 변경 |
...
> **스택드 슬래시-스킬 호출 (2.1.199)**: `/skill-a /skill-b` 처럼 슬래시-스킬을 연속으로 나열하면 **최대 5개**까지 선행 로드하여 순차 실행할 수 있습니다.
...
| `disallowed-tools` | 아니오 | 스킬이 활성인 동안 제거할 도구 (2.1.152). 예: 읽기 전용 스킬에서 `Edit`, `Write` 차단 |
