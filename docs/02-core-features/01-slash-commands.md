...
- 변경은 즉시 적용됩니다
- **현재 세션에만 적용**됩니다. 새 세션의 기본 모델로 지정하려면 모델 선택기에서 **`d` 키**를 누르세요 (2.1.144)
...
| **번들** | `/code-review` | `[노력\|--comment]` | 코드 리뷰(정확성 버그 포함) — 이전 `/simplify` |
| | `/batch` | `<설명>` | 대규모 병렬 변경 |
