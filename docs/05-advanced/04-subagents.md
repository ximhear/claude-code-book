...
> **`/agents` 위저드 제거 (2.1.198)**: 서브에이전트가 기본 백그라운드 실행으로 바뀌면서(아래 참고) 대화형 **생성 위저드는 제거**되었습니다. 커스텀 에이전트는 `.claude/agents/`에 마크다운 파일을 직접 작성해 정의하세요.
...
> **기본 백그라운드 실행 (2.1.198)**: 서브에이전트는 이제 **기본적으로 백그라운드에서 실행**됩니다. 메인 세션은 서브에이전트를 스폰한 뒤 대기하지 않고 계속 진행하며, `/tasks`로 상태를 확인하고 필요할 때 결과를 회수합니다.

> **다단 중첩 (2.1.172)**: 서브에이전트가 자체 서브에이전트를 스폰할 수 있으며, **최대 5단계**까지 중첩됩니다. 대규모 작업을 계층적으로 분해할 때 활용합니다.

### 포그라운드

서브에이전트가 완료될 때까지 메인 세션이 대기하도록 할 수도 있습니다:
...
| **실행** | 기본 백그라운드 (2.1.198), 필요 시 포그라운드 대기. 최대 5단계 중첩 (2.1.172) |
