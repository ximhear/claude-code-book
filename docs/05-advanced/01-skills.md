...
명령 본문에서 숫자 앞의 `$`를 인자 치환이 아닌 **리터럴 문자**로 쓰려면 `\$`로 이스케이프합니다:

```markdown
---
name: price-check
description: "가격을 점검합니다"
---

기준 가격은 \$5, 상한은 \$25입니다. $ARGUMENTS 항목을 이 범위와 비교해주세요.
```

`\$5`, `\$25`는 위치 인자 `$5`/`$25`로 치환되지 않고 그대로 `$5`, `$25`로 출력됩니다.
...
플러그인에 포함된 스킬은 플러그인 설치 시 자동으로 사용 가능합니다. 자세한 내용은 [25장: 플러그인](07-plugins.md)을 참고하세요.

### 중첩 디렉토리 로드 (2.1.178)

프로젝트 하위의 **중첩된 `.claude/skills`·`.claude/` 디렉토리**에 있는 스킬·에이전트·워크플로우·output-style도 로드됩니다. 서로 다른 디렉토리에 같은 이름의 스킬이 있어 충돌하면 **`<디렉토리>:<이름>`** 형식으로 구분해 호출합니다.

```
> /frontend:review    # frontend 디렉토리의 review 스킬
> /backend:review     # backend 디렉토리의 review 스킬
```

### 번들 스킬 숨기기

내장 번들 스킬·워크플로우·내장 슬래시 커맨드를 숨기려면 `disableBundledSkills: true`(또는 환경 변수 `CLAUDE_CODE_DISABLE_BUNDLED_SKILLS`)를 설정합니다 (2.1.169). 자세한 내용은 [10장: settings.json](../03-configuration/02-settings.md)을 참고하세요.
...
> **새 스킬 즉시 인식 (2.1.152)**: 기존 스킬 *수정*은 자동 반영되지만, 디렉토리에 **새 스킬을 추가**하면 `/reload-skills` 커맨드로 재스캔해야 같은 세션에서 인식됩니다. `SessionStart` 훅에서 `reloadSkills: true`를 반환해도 동일하게 재스캔됩니다.

---

## 스택드 슬래시-스킬 호출 (2.1.199)

여러 슬래시-스킬을 한 줄에 연속으로 나열하면 **최대 5개**까지 선행 로드하여 순서대로 실행합니다:

```
> /lint /test /commit
```

각 스킬이 입력 순서대로 실행되므로, 자주 함께 쓰는 스킬 조합을 한 번에 호출할 때 편리합니다.
